...
const GITHUB = {
    ...
    exports.handler = async function principal(context, event, callback) {
        ...
        let username = escape(event.Body);
        let usrCommits;
        ...
        if(typeof usrCommits != 'undefined'){
            if(typeof repos != 'undefined'){
                twiml.message(`Hi again, the user!,  ${username} has: ${usrCommits} days-in-row and the active repository of ${username} is  ${JSON.stringify(repos[0].name)}`);
                callback(null, twiml);
                return
            }
        else{
                twiml.message(`The user entered does not exist or has not made any activity recently.`);
                callback(null, twiml);
                return
            }
        }
